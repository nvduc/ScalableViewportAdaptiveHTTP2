...
void print_new_seg(int cur_seg,int index) {
  std::cout << std::endl << "Respond seg # " << cur_seg <<"of video "<<index+1<< " bitrate " << bitrate[index];
...
void push_remaining_files(const response *res, int face_index, int curr_seg) {
  ...
  push->on_close([res,face_index, curr_seg](uint32_t error_code) {
    if (!error_code) {                    // if CANCEL is not sent
     ...
     // server_seg[region] = server_seg[region] + 1;
      std::cout << "Sent seg #" << curr_seg <<" face "<< face_index <<" bitrate " << bitrate[face_index]
  ...
  push->write_head(200);
  push->end(file_generator("./vr_rate/"+std::to_string(segment_duration) + "s/Face" + std::to_string(face_index) + "/" + bitrate[face_index]));
  on_pushing_in_periodic_mode[face_index] = true;
}
...
void push_file(
  ...
  if (on_pushing_in_periodic_mode[face_index]) { return; }

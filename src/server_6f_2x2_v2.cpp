...
int main(int argc, char *argv[]) {
  ...
  server.handle("/face0/", [](const request &req, const response &res) {
    ...
    boost::split(temp, strs[1], boost::is_any_of("e"));
    int face_index = std::stoi(temp[1]);
    boost::split(temp, strs[2], boost::is_any_of("="));
    int bitrate = std::stoi(temp[1]);
    boost::split(temp , strs[3] , boost::is_any_of("="));
    int num = std::stoi(temp[1]);
    int curr_seg = 1;

    std::cout << "face_index: " << face_index << ", birate: " << bitrate << ", num: " << num << std::endl;

    res.write_head(200);
    res.end(file_generator("./vr_rate/6f_2x2/"+std::to_string(segment_duration) + "s/Face" + std::to_string(face_index) + "/" + std::to_string(bitrate)));
  });
